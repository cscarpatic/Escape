  ...
  RoadNetwork=class RoadNetwork extends BaseRoadNetwork{
    ...
    enforceTopology(){
      ...
      for(const ring of groups.values()){
        ...
        for(const p of this.paths){const a=ring.has(p.nodeA),b=ring.has(p.nodeB);if(a&&!b)external.add(p.nodeB);if(b&&!a)external.add(p.nodeA);}
        const ext=[...external];
        for(let i=0;i<ext.length;i++)for(let j=i+1;j<ext.length;j++)for(const p of this.paths){
      ...
      for(let i=0;i<candidates.length;i++){
        ...
        for(let j=i+1;j<candidates.length;j++){
          ...
          const pa=priority(a),pb=priority(b),loser=pa===pb?(a.length<b.length?a:b):(pa<pb?a:b);
          remove.add(loser.id);this.integrity.removedCrossings++;if(loser===a)break;
        }
      }
