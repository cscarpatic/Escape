(() => {
  ...
  function pathsCross(a,b){if(!bboxHit(a,b))return false;for(let i=1;i<a.points.length;i++)for(let j=1;j<b.points.length;j++)if(cross(a.points[i-1],a.points[i],b.points[j-1],b.points[j]))return true;return false;}
  RoadNetwork=class RoadNetwork extends BaseRoadNetwork{
    ...
    enforceTopology(){
      const remove=new Set(),groups=new Map();
      for(const n of this.nodes.filter(n=>n.type==='roundabout')){const key=n.id.replace(/R[NEWS]$/,'');if(!groups.has(key))groups.set(key,new Set());groups.get(key).add(n.id);}
      for(const ring of groups.values()){
        const external=new Set();for(const p of this.paths){const a=ring.has(p.nodeA),b=ring.has(p.nodeB);if(a&&!b)external.add(p.nodeB);if(b&&!a)external.add(p.nodeA);}const ext=[...external];
        for(let i=0;i<ext.length;i++)for(let j=i+1;j<ext.length;j++)for(const p of this.paths){if(p.feature==='roundabout')continue;if((p.nodeA===ext[i]&&p.nodeB===ext[j])||(p.nodeA===ext[j]&&p.nodeB===ext[i])){remove.add(p.id);this.integrity.removedBypasses++;}}
      }
      const candidates=this.paths.filter(p=>!remove.has(p.id));
      for(let i=0;i<candidates.length;i++){
        const a=candidates[i];if(remove.has(a.id))continue;
        for(let j=i+1;j<candidates.length;j++){
          const b=candidates[j];if(remove.has(b.id)||sharesNode(a,b)||levelOf(a)!==levelOf(b))continue;if(a.kind==='ramp'&&b.kind==='ramp'&&a.stage===b.stage)continue;if(!pathsCross(a,b))continue;
          const pa=priority(a),pb=priority(b),loser=pa===pb?(a.length<b.length?a:b):(pa<pb?a:b);remove.add(loser.id);this.integrity.removedCrossings++;if(loser===a)break;
        }
      }
      if(remove.size)this.paths=this.paths.filter(p=>!remove.has(p.id));const valid=new Set(this.paths.map(p=>p.id));for(const n of this.nodes)n.edges=n.edges.filter(id=>valid.has(id));
